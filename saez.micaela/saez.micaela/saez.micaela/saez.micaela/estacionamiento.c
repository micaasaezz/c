...
void hardcodea4estacionamientos(eProfesor profesores[])
{
    ...
    profesores[0].horaEntrada = 8;
    profesores[2].estacionadoAhora = 1;
    profesores[2].horaEntrada = 9;
    profesores[3].estacionadoAhora = 1;
    profesores[3].horaEntrada = 10;
    profesores[5].estacionadoAhora = 1;
    profesores[5].horaEntrada = 11;
}
...
int altaAutomovil(eAutomovil automovil[], int posicion, eProfesor profesor[], int tamP, int tamA)
{
    ...
    printf("Ingrese el id del profesor que quiere dar de alta:\n");

    for(i=0; i<tamP; i++)
    {
        if(profesor[i].estado == 0)
        {
            printf("%d\n", profesor[i].idIdentificacion);
        }
    }
    scanf("%d", &auxId);
...
void ingresoPlayaEstacionamiento(eProfesor profesores[], int tamP)
{
    ...
    printf("Elija nombre del profesor para ingresar:\n");
    for(i=0; i<tamP; i++)
    {
    ...
    for(i=0; i<tamP; i++)
    {
        if(strcmp(auxNombre, profesores[i].nombre) == 0)
        {
            ...
            scanf("%d", &profesores[i].horaEntrada);
            while(profesores[i].horaEntrada > 24)
            {
...
int egresoPlayaEstacionamiento(eProfesor profesores[], int tamP)
{
    ...
    char sacarAuto;
    int respuesta=1;
    int cantHorasEstacionado, precioEstacionamiento;


    printf("Elija nombre del profesor para el egreso:\n");
    for(i=0; i<tamP; i++)
    {
        if(profesores[i].estacionadoAhora == 1 && profesores[i].estado ==1)
        {
            printf("%s\n", profesores[i].nombre);
        }
    }
    ...
    for(i=0; i<tamP; i++)
    {
        if(strcmp(auxNombre, profesores[i].nombre) == 0)
        {
            ...
            {
                printf("Ingrese la hora:");
                scanf("%d", &profesores[i].horaSalida);
                while(profesores[i].horaSalida < profesores[i].horaEntrada || profesores[i].horaSalida > 24)
                {
                     printf("Error, reingrese hora:");
                     scanf("%d", &profesores[i].horaSalida);
                }
                ...
                cantHorasEstacionado = profesores[i].horaSalida - profesores[i].horaEntrada ;
                printf("Usted estacion%c por %d horas\n", 162,cantHorasEstacionado);
                if(cantHorasEstacionado < 4)
                {
                    printf("No debe pagar cargos extra.\n");
                }
                else
                {
                    precioEstacionamiento = (cantHorasEstacionado - 4)*10;
                    printf("Debe pagar %c%d por usar el estacionamiento.\n", 36, precioEstacionamiento);
                }
